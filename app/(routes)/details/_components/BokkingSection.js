...
function BokkingSection({ children ,business }) {
  ...
  const saveBooking=()=>{
    GlobalApi.createNewBooking(business.id,moment(date).format('DD-MMM-yyyy'),
        selectedTime,data.user.email,data.user.name)
        .then(resp=>{
            console.log(resp);
            if(resp){
                setDate('');
                setSelectedTime('');
                toast('Service Bokked sucessfully!')
                // Toast Message
            }
  ...
  const isSlotBooked=(time)=>{
    return bookSlot.find(item=>item.time==time)
  }
        ...
        <SheetContent className="overflow-auto">
            ...
            <SheetDescription>
              ...
              <div className="flex justify-center">
                <Calendar
                  ...
                  selected={date}
                  onSelect={setDate}
                  className="rounded-md border text-gray-500"
                />
                </div>
              </div>
              {/* TimeSlot Picker */}
          ...
          <SheetFooter className='mt-5'>
              <SheetClose asChild>
                ...
                <Button variant="destructive" className="">Close</Button>
